...
class JournalpostService(
    ...
    fun getDokument(journalpostId: String, dokumentInfoId: String, variantFormat: String): DokumentDto {
        val dokumentResponse = when {
            tokenService.isUserLoggedInAsPerson() ->
                safSelvbetjeningClient.hentDokument(journalpostId, dokumentInfoId, variantFormat)
        ...
        dokumenter.mapNotNull {
            it.dokumentvarianter.let { varianter ->
                varianter.firstOrNull { variant ->
                    variant.variantformat == JournalpostSafSelvbetjeningDto.Dokument.DokumentVariant.Variantformat.SLADDET
                } ?: varianter.firstOrNull { variant ->
                    variant.variantformat == JournalpostSafSelvbetjeningDto.Dokument.DokumentVariant.Variantformat.ARKIV
                }
            }?.let { variant ->
