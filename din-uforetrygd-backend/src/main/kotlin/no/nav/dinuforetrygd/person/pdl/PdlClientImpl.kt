...
class PdlClientImpl(
    ...
    override fun performQuery(query: PdlPersonQuery): PdlPerson {
        return performQuery(query, tokenService.getEgressToken(scope, audience, query.variables.ident, AppId.PDL))
    }

    override fun performQueryWithElevatedPriveleges(query: PdlPersonQuery): PdlPerson {
        return performQuery(query, azureAdService.retrieveClientCredentialsToken(listOf(scope)))
    }

    override fun performQuery(query: PdlPersonQuery, token: String?): PdlPerson {
    ...
    companion object {
        ...
        private val logger: Logger = LoggerFactory.getLogger(PdlClientImpl::class.java)
    }
}